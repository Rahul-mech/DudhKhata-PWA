...
export function mergeEntriesByDate(entries: MilkEntry[]): DayRow[] {
  ...
  for (const [entryDate, list] of map) {
    ...
    rows.push({
      entryDate,
      morningLitres: round2(morningParts.reduce((s, p) => s + p.litres, 0)),
      morningFat: weightedFat(morningParts),
      eveningLitres: round2(eveningParts.reduce((s, p) => s + p.litres, 0)),
...
export function buildContactBillHtml(opts: {
  ...
  return `
    ...
    <p class="muted">${escapeHtml(contact.name)}${
    ...
    <p class="muted">Generated by DudhKhata · Advances deducted from milk total</p>
  `;
}
...
export function buildMonthSummaryText(opts: {
  ...
  for (const r of opts.rows) {
    lines.push(`${r.name}: ${formatLitres(r.litres)} · ${formatInr(r.settlement)}`);
  }
  lines.push(`----------------`);
